...
const defaultState = {
  ...
  plans: {
    // userId: { 'YYYY-MM-DD': { mode, start, focus, wellness, notes } }
  },
  ...
  learningHistory: {
    // userId: [{ name, url, minutes, completedAt, ts }]
  },
  ...
  // Policies & Compliance
  policies: [
    // { id, title, version, publishedAt:'YYYY-MM-DD', url, category, required:true }
  ],
  acknowledgments: {
    // userId: [policyId]
  },
  // Assets tracking
  assets: [
    // { id, tag, type, assignedTo:userId|null, status, notes }
  ]
  ,
  requests: [
...
export const Store = {
  ...
  setCurrentUser(user) {
    ...
    // For Supabase users, we might want to store more than just the ID
    // or ensure they exist in our local 'users' list if needed for demo purposes.
    const s = loadState();
    const existing = s.users.find(u => u.email === user.email);
    if (!existing) {
  ...
  addPolicy(policy) {
    ...
    const exists = s.policies.find(p => (p.id || '') === id);
    const entry = { id, title: policy.title || 'Policy', version: policy.version || 'v1', publishedAt: policy.publishedAt || new Date().toISOString().slice(0, 10), url: policy.url || '', category: policy.category || 'General', required: policy.required !== false };
    if (exists) { Object.assign(exists, entry); }
    else { s.policies.push(entry); }
    saveState(s);
    return id;
  },
  getPolicies() { const s = loadState(); return s.policies || []; },
  acknowledgePolicy(userId, policyId) {
    const s = loadState();
    s.acknowledgments = s.acknowledgments || {};
    s.acknowledgments[userId] = s.acknowledgments[userId] || [];
    const list = s.acknowledgments[userId];
    if (!list.includes(policyId)) list.push(policyId);
    saveState(s);
    return true;
  },
  getAcknowledgments(userId) { const s = loadState(); return (s.acknowledgments || {})[userId] || []; },
  getPendingPoliciesForUser(userId) {
    const s = loadState();
    const all = s.policies || [];
    const ack = (s.acknowledgments || {})[userId] || [];
    return all.filter(p => p.required && !ack.includes(p.id));
  },
  ...
  addAsset(asset) {
    ...
    const entry = { id, tag: asset.tag || id, type: asset.type || 'Device', assignedTo: asset.assignedTo || null, status: asset.status || 'available', notes: asset.notes || '' };
    s.assets.push(entry);
    saveState(s);
    return id;
  },
  updateAsset(id, patch) {
    ...
    if (idx >= 0) { s.assets[idx] = { ...s.assets[idx], ...(patch || {}) }; saveState(s); return true; }
    return false;
  },
  assignAsset(id, userId) {
    return Store.updateAsset(id, { assignedTo: userId, status: 'assigned' });
  },
  getAssets() { const s = loadState(); return s.assets || []; },
  getAssetsForUser(userId) { const s = loadState(); return (s.assets || []).filter(a => a.assignedTo === userId); },

  addRequest(req) {
    const s = loadState();
    s.requests = s.requests || [];
    const id = req.id || `req-${Date.now()}`;
    const entry = { id, userId: req.userId, type: req.type || 'general', description: req.description || '', amount: Number(req.amount || 0), status: req.status || 'pending', createdAt: req.createdAt || new Date().toISOString(), notes: req.notes || '' };
  ...
  updateRequest(id, patch) {
    ...
    if (idx >= 0) { s.requests[idx] = { ...s.requests[idx], ...(patch || {}) }; saveState(s); return true; }
    return false;
  },
  updateRequestStatus(id, status) { return Store.updateRequest(id, { status }); },
  getRequests(userId) { const s = loadState(); return (s.requests || []).filter(r => r.userId === userId); },
  getAllRequests() { const s = loadState(); return s.requests || []; },

  setPlan(userId, dateStr, plan) {
    const s = loadState();
    s.plans = s.plans || {};
    s.plans[userId] = s.plans[userId] || {};
    s.plans[userId][dateStr] = { ...(s.plans[userId][dateStr] || {}), ...(plan || {}) };
    saveState(s);
    return true;
  },
  getPlan(userId, dateStr) {
    const s = loadState();
    return ((s.plans || {})[userId] || {})[dateStr] || null;
  },
  getPlans(userId) {
    const s = loadState();
    return (s.plans || {})[userId] || {};
  },
  ...
  getAllLeaves() { return loadState().leaves || []; },
  updateLeaveStatus(id, status, approverId, notes) {
    const s = loadState();
    const idx = s.leaves.findIndex(l => l.id === id);
    if (idx >= 0) {
      const l = s.leaves[idx];
      s.leaves[idx] = { ...l, status, approverId: approverId || l.approverId, approverNotes: notes || l.approverNotes, decidedAt: new Date().toISOString() };
      saveState(s);
      try {
        Store.addNotification({ type: `leave_${status}`, userId: l.userId, date: Store.todayStr(), message: `Your ${l.type} leave (${l.startDate} → ${l.endDate}) was ${status}` });
      } catch { }
      return true;
    }
    return false;
  ...
  addLearning(userId, entry) {
    const s = loadState();
    s.learningHistory = s.learningHistory || {};
    s.learningHistory[userId] = s.learningHistory[userId] || [];
    s.learningHistory[userId].unshift({ ...entry, ts: new Date().toISOString() });
    saveState(s);
    return true;
  },
  getLearningHistory(userId, opts = {}) {
    const s = loadState();
    let list = ((s.learningHistory || {})[userId] || []).slice();
    const lastNDays = Number((opts || {}).lastNDays || 0);
    if (lastNDays > 0) {
      const end = new Date();
      const start = new Date(); start.setDate(end.getDate() - (lastNDays - 1));
      const sstr = start.toISOString().slice(0, 10);
      const estr = end.toISOString().slice(0, 10);
      list = list.filter(e => {
        const d = (e.completedAt || e.ts || '').slice(0, 10);
