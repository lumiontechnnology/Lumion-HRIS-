...
function injectHeaderFooter() {
  ...
  // Scoped override: neutralize glass tokens on user dashboard footer only
  try {
    const isUserDash = /\/user-dashboard\.html$/i.test(location.pathname);
    if (isUserDash) {
      const style = document.createElement('style');
      style.textContent = `
        .glass-card{ backdrop-filter:none !important; background:transparent !important; border:none !important; box-shadow:none !important; }
        .glass-input{ backdrop-filter:none !important; background:#fff !important; border:1px solid #d1d5db !important; }
        .glass-input:focus{ box-shadow:none !important; border-color:#d1d5db !important; }
        .btn-quick:hover{ transform:none !important; box-shadow:none !important; }
      `;
      document.head.appendChild(style);
    }
  } catch { }

  // Global sanitizer: remove stray CSS token text nodes
  try {
    const patterns = [/\/\*\s*glass tokens\s*\*\//i, /\.glass-card\{/, /\.glass-input\{/, /\.btn-quick\{/, /box-shadow:\s*0 10px 24px/];
    const stripNode = (n) => {
      if (!n) return;
      if (n.nodeType === Node.TEXT_NODE) {
        const t = n.textContent || '';
        if (patterns.some(re => re.test(t))) n.textContent = '';
      }
    };
    const walk = (root) => { const kids = root.childNodes || []; for (let i = 0; i < kids.length; i++) { const k = kids[i]; stripNode(k); if (k.nodeType === Node.ELEMENT_NODE) walk(k); } };
    walk(document.body);
    const mo = new MutationObserver((muts) => { muts.forEach(m => { (m.addedNodes || []).forEach(n => { stripNode(n); if (n.nodeType === Node.ELEMENT_NODE) walk(n); }); }); });
