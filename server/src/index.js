  ...
  try {
    ...
    await pool.query(`create table if not exists public.jobs (
      ...
      created_at timestamptz not null default now()
        ...
        await pool.query(`alter table public.employees add column if not exists exit_date timestamptz;`);
        await pool.query(`create table if not exists public.employee_performance (
          id bigint generated by default as identity primary key,
...
app.get('/api/employees', async (req, res) => {
  try {
    ...
    let sql = 'select id, employee_id, name, job_id, job_title, status, exit_date, notes, years, skills, certs, source_company, created_at from public.employees';
    const vals = [];
    if(status){ sql += ' where status = $1'; vals.push(status); }
...
app.post('/api/performance', async (req, res) => {
  try {
    const p = req.body && req.body.performance ? req.body.performance : req.body;
    if(!p || !p.employee_id || !p.year || !p.quarter) return res.status(400).json({ error: 'performance_required' });
    await pool.query(
      'insert into public.employee_performance(employee_id, year, quarter, kpi_score, projects_completed, attendance_pct, bonus_awarded, notes) values ($1,$2,$3,$4,$5,$6,$7,$8) on conflict(employee_id, year, quarter) do update set kpi_score=excluded.kpi_score, projects_completed=excluded.projects_completed, attendance_pct=excluded.attendance_pct, bonus_awarded=excluded.bonus_awarded, notes=excluded.notes',
      [p.employee_id, p.year, p.quarter, p.kpi_score||0, p.projects_completed||0, p.attendance_pct||0, p.bonus_awarded||0, p.notes||null]
    );
    res.status(201).json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: 'server_error' });
...
app.get('/api/performance', async (req, res) => {
  try {
    const { employee_id, year, quarter } = req.query || {};
    const conds = [];
    const vals = [];
    if(employee_id){ conds.push('employee_id = $' + (vals.length+1)); vals.push(employee_id); }
    if(year){ conds.push('year = $' + (vals.length+1)); vals.push(parseInt(year)); }
    if(quarter){ conds.push('quarter = $' + (vals.length+1)); vals.push(parseInt(quarter)); }
    const where = conds.length ? (' where ' + conds.join(' and ')) : '';
    const q = await pool.query('select employee_id, year, quarter, kpi_score, projects_completed, attendance_pct, bonus_awarded, notes, created_at from public.employee_performance' + where + ' order by year desc, quarter desc', vals);
    res.json(q.rows);
  } catch (e) {
    res.status(500).json({ error: 'server_error' });
  }
...
app.put('/api/employees/:employee_id', async (req, res) => {
  try {
    ...
    const exit_date = b.exit_date || null;
    await pool.query('update public.employees set status = coalesce($2, status), exit_date = $3 where employee_id = $1', [employee_id, status, exit_date]);
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: 'server_error' });
  }
